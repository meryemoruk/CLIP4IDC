...
class CLIP4IDC(CLIP4IDCPreTrainedModel):
    ...
    def forward(
        ...
        if(self.task_config.do_semantic):
            ...
            sequence_emb, visual_emb_rgb, visual_emb_semantic, sequence_output, visual_output_rgb, visual_output_semantic = self.get_sequence_and_visual_output_sem(
                input_ids,
                token_type_ids,
                attention_mask,
                image_pairs,
        ...
        if self.training:
            ...
            if self._stage_one is True and self._stage_two is False:
                ...
                sim_loss1 = self.loss_fct(sim_matrix_text)
                ...
                image_mask = torch.ones(
                    visual_output_rgb.shape[0], visual_output_rgb.shape[1], device=visual_output_rgb.device
                ).long()

                decoder_scores, res_tuples = self._get_decoder_score(
                    visual_output_rgb, image_mask, input_caption_ids, decoder_mask, shaped=True
                )
    ...
    def get_sequence_and_visual_output_sem(
            self,
            input_ids,
            token_type_ids,
            attention_mask,
